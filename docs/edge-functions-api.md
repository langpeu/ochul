...
- `POST /study-rooms/{studyRoomId}/students`: 학생 생성, 6자리 출결 비밀번호 해시 저장
  - Body: `{ "name": "학생", "code": "A001", "pin": "123456", "gender": "unspecified", "ageGroup": "elementary", "avatarKey": "elementary_unspecified_01" }`
- `GET /students/{studentId}`: 학생 상세
- `PATCH /students/{studentId}`: 학생 수정
  - Body: `{ "name": "학생", "code": "A001", "status": "active", "gender": "unspecified", "ageGroup": "elementary", "avatarKey": "elementary_unspecified_01" }`
...
- `PUT /students/{studentId}/guardians`: 보호자 연락처, 관계, 카카오 수신 동의, 대표 연락처 저장
  - Body: `{ "guardians": [{ "name": "보호자", "phone": "01012345678", "relationship": "모", "kakaoOptIn": true, "consentConfirmed": true, "primaryContact": true, "deleted": false }] }`
  - `kakaoOptIn=true`인 경우 `consentConfirmed=true`가 필요하다.
  - `kakaoOptIn=false`는 수신 거부, `deleted=true`는 보호자 정보 삭제 요청으로 기록하고 카카오 발송 대상에서 제외한다.
...
- `GET /study-rooms/{studyRoomId}/notifications`: 카카오 발송 이력, 상태/학생 필터 지원
  - Body filter: `status`, `studentId`, `classId`, `dateFrom`, `dateTo`, `limit`
...
- Body filter: `category` (`all`, `student`, `class`, `kakao`), `studentId`, `classId`, `dateFrom`, `dateTo`, `limit`
