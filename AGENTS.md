...
- 학생 출결 비밀번호는 숫자 6자리이며 평문 저장 금지. 서버에는 해시만 저장한다.
- 선생님은 학생 출결 비밀번호를 조회할 수 없고 리셋만 할 수 있다.
- 학생 이름, 보호자 연락처, 출결, 납부 정보는 개인정보로 취급한다.
- 보호자 전화번호 저장 전 출결/수업 변경/납부 안내 목적을 고지하고 동의 확인 기록을 남긴다.
- 카카오 발송 로그와 감사 로그에는 보호자 전화번호 원문을 저장하지 않고 마스킹된 번호만 저장한다.
- 보호자가 수신 거부하거나 삭제를 요청할 수 있어야 한다.
...
- 수업 생성/수정 시 책상과 의자 배치도를 설정할 수 있어야 한다.
- 좌석 배치형 출석에서는 학생이 자기 아바타를 의자에 드래그앤드롭하고 비밀번호 검증 후 출석 처리한다.
- 학생 아바타는 남/여/미지정 및 초등/중등/고등 구분을 지원하되 실제 학생 사진은 사용하지 않는다.
...
- Edge Function API는 REST 스타일의 간결한 리소스/동작 이름을 사용한다.
